...
export interface BootstrapVerifierAgentOptions {
...
export async function createVerifierAgent(
  ...
  options: BootstrapVerifierAgentOptions = {},
) {
  ...
  const [
    { AnonCredsDidCommProofFormatService, AnonCredsModule },
    { AskarModule },
...
export async function bootstrapVerifierAgent(
  config: VerifierAgentConfig,
  options: BootstrapVerifierAgentOptions = {},
) {
  const agent = await createVerifierAgent(config, options)
