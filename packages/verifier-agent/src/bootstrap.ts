...
export function requireFinishedDid(result: DidCreateResult): string {
  ...
  if (didState.state === 'failed') {
    throw new Error(`Verifier bootstrap DID creation failed: ${didState.reason}`)
  }

  if (didState.state === 'action') {
    throw new Error(`Verifier bootstrap DID creation requires manual action: ${didState.action}`)
  }

  throw new Error('Verifier bootstrap DID creation did not finish yet')
}
